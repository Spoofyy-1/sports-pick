...
def _enrich(game: dict) -> dict:
    ...
    probs = model_predict.win_probabilities(game["home_team"], game["away_team"])
    out["model"] = probs
...
async def root():
    return {
        "ok": True,
        "service": "sports-pick",
        "model_trained": bool(model_predict.state().ratings),
...
async def model_reload():
    ...
    s = model_predict.state()
    return {"reloaded": True, "teams": len(s.ratings)}
